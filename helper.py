...
def most_successful(df, selected_sport):
    ...
    x = temp_df['Name'].value_counts().reset_index(name='Medals').head(15).merge(df, left_on='Name', right_on='Name',how = 'left')[['Name','Medals','Sport','region']].drop_duplicates('Name')
    x.rename(columns={ 'index': 'Name','count': 'Medals'}, inplace = True)
    return x

def yearwise_medal_tally(df,selected_country):
    temp_df = df.dropna(subset=['Medal'])
